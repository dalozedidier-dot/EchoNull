...
#!/usr/bin/env python3
"""
Patch EchoNull CI for Python 3.12+ (PEP 695) and fix coverage collection.

Observed failure patterns:
- Python 3.10 job fails with SyntaxError on `def perf_timer[...]` (PEP 695 is 3.12+).
- Python 3.12 job passes tests but coverage is 0% because CI runs
  `pytest --cov=src` while the repository is not a `src/` layout.

This script updates `.github/workflows/ci.yml`:
- Removes python-version entries < 3.12 from matrix lists.
...
def patch_python_matrix(lines: list[str]) -> tuple[list[str], bool]:
    """
    Patch python-version matrices written either as:
      python-version:
        - "3.10"
        - "3.12"
    or:
